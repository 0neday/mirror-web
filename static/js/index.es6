...
---
$(document).ready(() => {
var lei3Po8h = ["support", ["njcit", "cn"].join(".")].join("@");
$('a#eib1gieB')
	.text(lei3Po8h)
...
var mir_tmpl = $("#template").text(),
	...
	{
		'status': 'failed',
		'last_update': '2016-12-02 08:07',
		'name': "Kali",
		'url': 'https://mirrors.njcit.cn/kali',
		'upstream': '',
		'is_help':'true'
	},
	{
		'status': 'paused',
		'last_update': '2016-12-01 13:37',
		'name': "Mac OS",
		'url': 'https://mirrors.njcit.cn/mac',
		'upstream': '',
		'is_help':'true'
	}
	],
...
var vmMirList = new Vue({
	...
	methods: {
		...
		refreshMirrorList () {
			var self = this;
			$.getJSON("/static/sync.json", (status_data) => {
				...
				for(var k in mir_data) {
					...
					d.show_status = (d.status != "success_");
